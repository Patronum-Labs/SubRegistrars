...
contract TokenizedRegistrar is ERC721, Ownable {
    /// @notice Emitted when a subname is registered and tokenized.
    ...
    event SubnameRegistered(string name, bytes32 label, address owner);
    ...
    ) ERC721("Tokenized Registrar", "TREG") Ownable(initialOwner) {
    ...
    /// @notice Internal function to register a subname and mint a corresponding ERC721 token.
    ...
    function _registerName(
        ...
        emit SubnameRegistered(name, label, owner_);
    }

    /// @notice Internal function to register multiple subnames and mint corresponding ERC721 tokens.
